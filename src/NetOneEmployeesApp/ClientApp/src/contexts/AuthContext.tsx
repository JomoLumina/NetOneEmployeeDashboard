...
interface AuthContextValue extends AuthState {
  ...
  logout: () => void;
  register: (name: string, surname: string, emailAddress: string, phone: string, password: string) => Promise<void>;
}
...
export const AuthProvider: FC<AuthProviderProps> = ({ children }) => {
  ...
  const register = async (name: string, surname: string, emailAddress: string, phone: string, password: string) => {
    const endpoint = '/users/register';
    const data = JSON.stringify({ name, surname, emailAddress, phone, password });
    const response = await axios.post(endpoint, data);
    const user: User = response.data;
    history.push('/login');
    dispatch({
